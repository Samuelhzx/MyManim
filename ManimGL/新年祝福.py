...
class blessings(Scene):
    def construct(self):
        ...
        def yanhua(
            height, x, path_h, colorp, colorf, rad, blessings, shift, scale=1, len=None
        ):
            if len is None:
                len = rad - 0.1
            ...
            self.add(path)
            text = (
                Text(blessings, font="Smiley Sans", color=YELLOW, run_time=2)
                .shift(shift)
                .scale(scale)
            )
            g.add(text)
            self.play(
                path.animate.shift(UP * (4 + height)), rate_func=linear, run_time=2.5
            )
            self.play(
                ApplyMethod(path.set_opacity, 0, run_time=1.5),
                Flash(
                    path.get_start(),
                    colorf,
                    run_time=2,
        ...
        byebye = (
            Text("bye bye", font="Smiley Sans", color=YELLOW, run_time=2)
            .shift(UP)
            .scale(3)
        )
        hihi = (
            Text("hi hi", font="Smiley Sans", color=YELLOW, run_time=2)
            .shift(UP)
            .scale(3)
        )
